...
od.Device=function(e){var c=od.CommandType,b=this;this.type=od.DeviceType.DIGITAL;this.listeners=[];this.on=function(){this.setValue(1,!0)};this.off=function(){this.setValue(0,!0)};this.isON=function(){return 1==this.value};this.isOFF=function(){return 0==this.value};this.setValue=function(b,c){c="undefined"!==typeof c?c:!0;if(this.type==od.DeviceType.NUMERIC||this.value!=b)this.value=b,this.lastUpdate=(new Date).getTime(),this.manager&&this.manager.notifyDeviceListeners(this,c)};this.toggleValue=
this.toggle=function(){var b=0;0==this.value?b=1:1==this.value&&(b=0);this.setValue(b)};this.onChange=function(b){this.listeners.push(b)};(function(d){this.id=d.id;this.manager=od.deviceManager;for(var f in d)this[f]=d[f];for(var e in this.properties)this[e]=this.properties[e];this.actions.forEach(function(d){b[d]=function(){console.log("Calling remote action: "+d+", params: ",arguments);var a=[],l;for(l in arguments)a.push(arguments[l]);b.manager.send({type:c.ACTION,deviceID:b.id,action:d,params:a})}})}).call(this,
e)};od=od||{};od.connection={};od.ConnectionStatus={CONNECTING:1,CONNECTED:2,DISCONNECTING:3,DISCONNECTED:4,FAIL:6};
...
od.DeviceManager=function(e){function c(a,b){if(void 0!==l[a])for(var c=l[a],d=0;d<c.length;d++)if("function"===typeof c[d])try{c[d](b)}catch(e){console.log(e)}}function b(){for(var a=OpenDevice.devices.list(),b=[],c=0;c<a.length;c++){var e=new od.Device(a[c]);"undefined"!=typeof Object.observe?Object.observe(e,d):console.warn("Object.observe not supported in this browser.");b.push(e)}return b}function d(a){if(0<a.length&&"update"==a[0].type&&"value"!=a[0].name){var c=a[0].object;f.send({type:g.SET_PROPERTY,
deviceID:c.id,property:a[0].name,value:c[a[0].name]})}}var f=this;od.deviceManager=this;var h=od.Event,g=od.CommandType,a=[],l={};this.connection=e||od.connection;this.setValue=function(a,c){var b=f.findDevice(a);b&&b.setValue(c)};this.toggleValue=function(a){(a=f.findDevice(a))&&!a.sensor&&a.toggle()};this.send=function(a){f.connection.send(a)};this.addDevice=function(){};this.getDevices=function(){return a&&0<a.length?a:a=this.sync(!1)};this.findDevice=function(c){if(a)for(var b=0;b<a.length;b++){if(a[b].id==
c)return a[b]}else console.warn("Devices not loaded or empty !");return null};this.sync=function(k,d){a=null;a=b();(d||a&&0==a.length)&&f.send({type:g.GET_DEVICES,forceSync:d});!0===k&&c(h.DEVICE_LIST_UPDATE,a);return a};this.on=function(a,b){f.addListener(a,b)};this.onDeviceChange=function(a){f.addListener(od.Event.DEVICE_CHANGED,a)};this.onConnect=function(a){this.on(od.Event.CONNECTED,function(){var b=OpenDevice.getDevices();a&&a(b)})};this.addListener=function(a,b){void 0===l[a]&&(l[a]=[]);l[a].push(b)};
this.contains=function(a,b){null==b&&(b=f.getDevices());for(var c=0;c<b.length;c++)if("object"==typeof b[c]){if(a.id==b[c].id)return!0}else if(a.id==b[c])return!0;return!1};this.notifyDeviceListeners=function(a,b){b&&f.connection.send({type:a.type,deviceID:a.id,value:a.value});for(var d=0;d<a.listeners.length;d++)a.listeners[d](a.value);c(h.DEVICE_CHANGED,a)};f.connection.addListener({onMessageReceived:function(a,d){if(!g.isDeviceCommand(d.type)||a.getConnectionUUID()!=d.connectionUUID){if(g.isDeviceCommand(d.type)){console.log("Device changed in another client..");
var e=f.findDevice(d.deviceID);e&&e.setValue(d.value,!1)}d.type==g.GET_DEVICES_RESPONSE&&(e=b(),c(h.DEVICE_LIST_UPDATE,e))}},connectionStateChanged:function(a,b,d){console.log("DeviceManager._connectionStateChanged :"+b);c(h.CONNECTION_CHANGE,b);od.ConnectionStatus.CONNECTED==b&&c(h.CONNECTED,f.getDevices());od.ConnectionStatus.CONNECTED==b&&c(h.DISCONNECTED)}})};od=od||{};od.APP_ID_NAME="AppID";od.version="0.3.2";od.appID="*";od.serverURL=window.location.origin;
...
$.ajax({type:"GET",url:od.serverURL+b,headers:{"X-AppID":od.appID},async:!1}).responseText;return 0<b.length?JSON.parse(b):null},history:function(b,c){jQuery.ajax({headers:{Accept:"application/json","Content-Type":"application/json","X-AppID":od.appID},type:"POST",url:od.serverURL+"/device/"+b.deviceID+"/history",data:JSON.stringify(b),dataType:"json",async:!0,success:c})},findAppID:function(){od.appID=function(b){var c;c=window.location.search.substr(1).split("\x26");if(""==c)c={};else{for(var e=
{},h=0;h<c.length;++h){var g=c[h].split("\x3d",2);e[g[0]]=1==g.length?"":decodeURIComponent(g[1].replace(/\+/g," "))}c=e}return c[b]}(od.APP_ID_NAME);if(null!=od.appID)return od.appID;od.appID=function(b){b=("; "+document.cookie).split("; "+b+"\x3d");if(2==b.length)return b.pop().split(";").shift()}(od.APP_ID_NAME);if(null!=od.appID)return od.appID;window.localStorage&&(od.appID=window.localStorage.getItem(od.APP_ID_NAME));return od.appID}}}(),ODev=OpenDevice;
