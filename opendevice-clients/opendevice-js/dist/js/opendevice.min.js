...
od.ConnectionStatus.CONNECTED==c&&d(m.DISCONNECTED)}})};od=od||{};od.APP_ID_NAME="AppID";od.version="0.3.2";od.appID="*";od.serverURL=window.location.origin;
...
rest:function(a){a=$.ajax({type:"GET",url:od.serverURL+a,headers:{"X-AppID":od.appID},async:!1});if(200==a.status&&0<a.responseText.length)return JSON.parse(a.responseText);console.error("Rest fail, status ("+a.status+"): "+a.responseText);return null},history:function(a,c){jQuery.ajax({headers:{Accept:"application/json","Content-Type":"application/json","X-AppID":od.appID},type:"POST",url:od.serverURL+"/device/"+a.deviceID+"/history",data:JSON.stringify(a),dataType:"json",async:!0,success:c})},logout:function(a){return $.get(od.serverURL+
"/api/auth/logout",a)},findAppID:function(){od.appID=function(a){var c;c=window.location.search.substr(1).split("\x26");if(""==c)c={};else{for(var b={},d=0;d<c.length;++d){var h=c[d].split("\x3d",2);b[h[0]]=1==h.length?"":decodeURIComponent(h[1].replace(/\+/g," "))}c=b}return c[a]}(od.APP_ID_NAME);if(null!=od.appID)return od.appID;od.appID=function(a){a=("; "+document.cookie).split("; "+a+"\x3d");if(2==a.length)return a.pop().split(";").shift()}(od.APP_ID_NAME);if(null!=od.appID)return od.appID;window.localStorage&&
(od.appID=window.localStorage.getItem(od.APP_ID_NAME));return od.appID}}}(),ODev=OpenDevice;OpenDevice.devices={path:"/device",get:function(h){return OpenDevice.rest(OpenDevice.devices.path+"/"+h)},value:function(h,d){return null!=d?OpenDevice.rest(OpenDevice.devices.path+"/"+h+"/value/"+d):OpenDevice.rest(OpenDevice.devices.path+"/"+h+"/value")},list:function(){return OpenDevice.rest(OpenDevice.devices.path+"/list")},sync:function(){return OpenDevice.rest(OpenDevice.devices.path+"/sync")}};