...
(function(root, factory) {
				...
				compile: function($tplElem) {
					...
					return function(scope, $elem, attrs, gridster) {
						...
						var onResize = gridsterDebounce(function onResize() {
							//var $this = $(this);
							//console.log("w:" + $this.css("width") +", h: " + $this.css("height"), this);
							resize();
							$timeout(function() {
								scope.$apply();
							});
	 ...
	 */
	.directive('gridsterItem', ['$rootScope', '$parse', 'GridsterDraggable', 'GridsterResizable', 'gridsterDebounce',
		function($rootScope, $parse, GridsterDraggable, GridsterResizable, gridsterDebounce) {
			return {
				scope: true,
				restrict: 'EA',
				...
				link: function(scope, $el, attrs, controllers) {
					...
					var debouncedTransitionEndPublisher = gridsterDebounce(function() {
						scope.$apply(function() {
							if (gridster.resizable && gridster.resizable.stopTransition) {
								gridster.resizable.stopTransition($el); // This will provide resize(stop) after animation
							}
